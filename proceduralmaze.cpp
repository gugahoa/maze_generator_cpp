...
void ProceduralMaze::generateLight() {
	...
	while (!check_stack.empty()) {
		...
		if (std::find(illuminated_cells.begin(), illuminated_cells.end(), check_pos) == illuminated_cells.end()) {
			auto diag_cells = getDiagCells(check_pos, Tile::EMPTY);
...
std::vector<std::pair<int, int>> ProceduralMaze::getDiagCells(std::pair<int, int> center, Tile tile_state) {
	std::vector<std::pair<int, int>> neighboors;
	for (int x = -1; x <= 1; x = x + 2) {
		for (int y = -1; y <= 1; y = y + 2) {
			try {
				auto n_cell = std::make_pair(center.first + x, center.second + y);
				if (grid.at(n_cell) == tile_state) {
...
void ProceduralMaze::fogPrint()
{
	for (int y = 0; y < height; ++y) {
		for (int x = 0; x < width; ++x) {
			...
			bool erase_char = true;
			auto adj = getAdjCells(pos, Tile::LIGHT, 1);
			for (auto n_pos : adj) {
				if (grid[n_pos] == Tile::LIGHT) {
					erase_char = false;
				}
			}

			auto neighboors = getDiagCells(pos, Tile::LIGHT);
			if (neighboors.size() == 0 && erase_char) {
				c = ' ';
			}
